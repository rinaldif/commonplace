...
function render(container) {
  ...
  const starterBanner = el('div', { 
    ...
    style: 'background: var(--color-accent-soft); padding: 1rem; border-radius: 8px; margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between; font-size: 0.8rem; border: 1px dashed var(--color-accent);',
    hidden: !store.get('isStarterMode')
  });
  ...
  // ──── Loading state ────
  const loadingEl = el('div', { class: 'loading-state', hidden: !store.get('isLoadingQuotes') });
  loadingEl.appendChild(el('div', { class: 'spinner' }));
  loadingEl.appendChild(el('span', {}, 'Loading quotes...'));
  ...
  function updateLoading() {
    loadingEl.hidden = !store.get('isLoadingQuotes');
  }
  ...
  unsubs.push(store.subscribe('isLoadingQuotes', updateLoading));
...
async function copyQuote() {
  ...
  const index = store.get('quoteIndex');
  if (index < 0) return;
  const { formatQuoteForCopy } = await import('../utils/format.js');
